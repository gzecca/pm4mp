#'@title Draw an "independent hot tree" with a graphical summary of multiple
#'  'nodesigl' analysis
#'
#'@description This function draws a user-selected "independent hot tree",
...
#'  'hmpp' function.
#'
#'@details The output files of the 'nodesigl_harvesteR' function are assumed to
...
#'
#'@param tftype Font type (default="i"). See the 'plotSimmap' function in the
#'  package 'phytools' for details.
#'
#'@param ncex Numeric, specifying the expansion applied to the symbols drawn at
#'  generic "hot nodes"(i.e, black filled circles). Default value is ncex= 0.6.
#'
#'@param br Logical, indicating whether best ranking "hot nodes" should be
#'  marked with a different symbol or not. Default is TRUE.
#'
#'@param br.ncex Numeric, specifying the expansion applied to the symbols drawn
#'  at best ranking "hot nodes". Default value is ncex= 0.8.
#'
#'@param br.pch Numeric, which specifies the symbol to be drawn corresponding to
...
#'  available options.
#'
#'@param uom Logical, specifying whether the unit of measurement of branch
#'  lengths must be added to the scale bar or not (default= TRUE).
#'
#'@param branch.unit Character string, specifying the unit used to measure
#'  branch lengths (default="Ma"). Ignored when uom=FALSE.
#'
#'@param autolabel Logical, specifying whether the function should automatically
...
#'
#'@param ttype Tree type. See the 'plotSimmap' function in the package
#'  'phytools' for details. (Defaul= "phylogram" or "fan", depending on the tree
#'  size).
#'
#'@param tfsize Numeric, specifying the relative font size for tip labels.
#'
#'@param arrow Logical, specifying whether known medicinal plants should be
#'  indicated by arrows in the final graph or not (default= TRUE).
#'
#'@param par.arrow List, including graphical parameters to modify the aspect of
...
#'
#'@param par.legend List, including graphical parameters internally passed to
#'  the 'add.simmap.legend' function of the 'phytools' package. Please refer to
#'  the 'add.simmap.legend' help for details and options.
#'
#'@param ... Additional arguments internally passed to the 'plotSimmap'
...
#'  available options.
#'
#'@return A tree with colored branches and symbols drawn at the "hot nodes" is
...
#' - Revell L (2024). “phytools 2.0: an updated R ecosystem for phylogenetic
...
#'
#'@seealso [nodesigl_harvesteR()], [hmpp()]
#'
...
hot_tree_painteR<-function(inputTrees,inputNodes,import.probs=FALSE, inputProbs,
                  indtr,tftype="i",ncex=0.6,br=TRUE, br.ncex= 0.8,br.pch=8,
                  uom=TRUE, branch.unit="Ma", autolabel=TRUE,ttype=NULL,
                  tfsize=NULL,arrow=TRUE, par.arrow =NULL, par.legend=NULL, ...){
  #Import and check
  if (!autolabel && (is.null(ttype)||is.null(tfsize))){
    stop("Please provide values for both ttype and tfsize parameters.")
  }
  TR<-ape::read.tree(file=inputTrees)
  if(inherits(TR,"phylo")){
    TR<-phytools::as.multiPhylo(TR)}
  if(indtr> length(TR)){
    stop("The value assigned to the indtr parameter is out of range.
         Please choose an appropiate value.")
  }
  htree<-TR[[indtr]]
  input<-utils::read.csv(inputNodes)
  if(!autolabel){
    tfsize<-tfsize; ttype<-ttype
  }

  #autolabel
  if(autolabel){
    fun<-ifelse(ape::Ntip(htree)>50,"power","exp")
    switch(fun,
      exp={tfsize<-1.540918*0.9824^ape::Ntip(htree)
      ttype<-"phylogram"},
      power={tfsize<- 6.581219*ape::Ntip(htree)^-0.60004
      ttype<-"fan"})
    print(paste0("tfsize parameter was set to: ", as.character(round(tfsize, digits=5))))
  }

  #import probability
  if(import.probs){
    probs<-utils::read.csv(inputProbs)
    tokeep<-probs$species %in% htree$tip.label
    probs<-probs[tokeep,]         #discarding the random outgroup
    ord<-match(htree$tip.label, probs$species)
    probs<-probs[ord,]            # making sure tips and probabilities are in the correct order
    htree$tip.label <-mapply(\(x,y) paste(x,format(trunc(y*100)/100,nsmall = 2) , sep=" "),
                             x= htree$tip.label, y= probs$hsp_p1)
  }

  #Split input by hot trees
  intN<-lapply((ape::Ntip(htree)+1):(ape::Ntip(htree)+ape::Nnode(htree)), \(x) x)
  names(intN)<-htree$node.label
  index<- factor(input$HotRoot, levels=unique(input$HotRoot))
  insplit<-split(input, f=index)

  #Find hot nodes and hot nodes bestrank
  NNumb<-as.numeric(sapply(insplit[[indtr]]$HotNodes, \(x) intN[[x]]))
  Num.best.rank<-NNumb[unlist(insplit[[indtr]]$bestrank)]
  Num.not.best<-setdiff(NNumb, Num.best.rank)

  #Setting colors for branches
  HNdesc<-sapply(NNumb, \(x) phytools::getDescendants(htree, x))
  HNlevel<-c()
  for (i in NNumb){
    lev<-1+sum(sapply(HNdesc, \(x) i %in% x))
    HNlevel<-c(HNlevel,lev)
  }
  myramp<-grDevices::colorRampPalette(c("#e4e4e4","#FFFF66","#FFCC00", "#FF9900","#FF0000"))
  cols<-myramp(max(HNlevel))
  cols<-c(cols,"#780032")
  names(cols)<-cols
  for (i in NNumb) {
    htree<-phytools::paintSubTree(htree, node=i, state= cols[HNlevel[which(NNumb == i)]])
  }
  #Find position of DRTs
  MedTaxa<- unlist(strsplit(insplit[[indtr]]$MedTaxa[[1]], split=" / "))
  DRTnumb<-as.numeric(unlist(sapply(MedTaxa, grep, htree$tip.label)))
  #Plotting
  htree<-phytools::paintBranches(htree,DRTnumb,"#780032")
  phytools::plotSimmap(htree,cols,pts=FALSE,type=ttype, fsize= tfsize, ftype=tftype,...)
  ape::nodelabels (node= Num.not.best,frame ="none", col = "black",bg ="black",
                   pch =16, cex=ncex)
  if(isTRUE(br) && length(Num.best.rank)>0){
    ape::nodelabels (node= Num.best.rank,frame ="none", col = "black",bg ="black",
                     pch =br.pch, cex=br.ncex)
  }else if (isTRUE(br) && length(Num.best.rank)== 0){
  print('WARNING: no "hot nodes" with the best ranking found!', quote=FALSE)}
  if(arrow){
    pararrow<-c(list(tree=htree, tip= DRTnumb, col= "#780032"), par.arrow )
    do.call(what=phytools::add.arrow, args=pararrow)
  }
  parlegend<-c(list(colors=cols,leg= c(paste("nhn:",1:max(HNlevel)), "Med species")), par.legend)
  do.call(what=phytools::add.simmap.legend, args=parlegend)
  ape::add.scale.bar(cex = 0.7, font = 2, ask=TRUE)
  if(uom){
    graphics::text(graphics::locator(1),branch.unit, cex = 0.7, font = 2)
  }
}
