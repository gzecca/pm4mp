#'
#' @title Preparing the input 'tree file' for 'nodesigl' analysis (Phylocom)
#'
#' @description This function prepares the input tree to be used with the
#'   software Phylocom (Webb et al. 2008) starting from a tree provided by the
#'   user.
#'
#' @details The functions assumes that the user-provided tree has correctly
...
#'
#' @param tree A user-provided tree in Newick format whose tips are assumed to
#'   be correctly labelled. See 'clean_and_match' function for details and
#'   options.
#'
#' @param fileout Character string specifying the suffix to add to the output
#'   files. This can be the same name as the input tree file or any other name
#'   chosen by the user.
#'
#' @return Two tree files are saved in Newick format, which include modified
...
#'
#' @seealso [clean_and_match()], [sample4nodesigl()]
#'
...
tree4nodesigl<-function(tree, fileout){
  mytree<-ape::read.tree(tree)
  rnodes<-paste0("ApeNumb.", (ape::Ntip(mytree)+1):(ape::Ntip(mytree) + ape::Nnode(mytree)))
  if(is.null(mytree$node.label)){
    mytree$node.label[1:ape::Nnode(mytree)]<-""}
  logic <-mytree$node.label==""|mytree$node.label=="NA"|is.na(mytree$node.label)
  mytree$node.label[logic]<-rnodes[logic]
  phy<-mytree
  phy$edge.length <- NULL
  ape::write.tree(phy, file = paste0("phylo_nobl_",fileout),
                  append = FALSE, tree.names = FALSE)
  ape::write.tree(mytree, file = paste0("phylo_bl_",fileout),
